...
function mapRowToPublic(row: PatientRow): PatientPublic {
  ...
  return {
    ...
    name:              decryptOptional(row.name) ?? 'Nome indisponível',
...
function mapRowToSummary(row: PatientRow): PatientSummary {
  ...
  const rawName = decryptOptional(row.name) ?? 'Nome indisponível';
...
export async function getDuplicatePatients(clinicId: string): Promise<DuplicateGroup[]> {
  ...
  return result.rows.map((row) => ({
    ...
    patients: [
      {
        id:         row.patient1_id,
        name:       decryptOptional(row.p1_name) ?? 'Nome indisponível',
      ...
      {
        id:         row.patient2_id,
        name:       decryptOptional(row.p2_name) ?? 'Nome indisponível',
