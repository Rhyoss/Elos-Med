...
import {
  SheetRoot as Sheet, SheetContent, SheetHeader, SheetTitle, SheetFooter,
...
function FieldTip({ children }: { children: React.ReactNode }) {
  // Usa o composto `<Tooltip content={...}>{trigger}</Tooltip>` — passar
  // <TooltipTrigger>+<TooltipContent> como children quebra o Slot interno
  // (React.Children.only no `<TooltipTrigger asChild>{children}</TooltipTrigger>`).
  return (
    <Tooltip content={<span className="max-w-xs text-xs block">{children}</span>}>
