/**
 * Processor `supply-stock-daily` — Prompt 12.C / Job 1.
 *
 * Dispatcher runs every 15 minutes and fires processing for clinics where the
 * local time is in the 06:00–06:15 window.
 *
 * Per clinic:
 *   1. Acquires Redis lock 'lock:daily-stock-check:{clinicId}:{YYYY-MM-DD-local}'
 *      (TTL 30 min) — ensures at most one execution per clinic per calendar day.
 *   2. Recalculates days_to_expiry and expiry_alert_level for all active lots
 *      (batches of 500, FOR UPDATE SKIP LOCKED).
 *   3. Expires lots where expiry_date < today.
 *   4. Emits idempotent lot_expiring alerts for critical lots.
 *   5. Verifies stock positions vs min_stock / reorder_point and emits
 *      low_alert / critical_alert accordingly.
 *
 * Idempotency: supply.alert_emissions_log UNIQUE (clinic_id, emission_key) where
 * emission_key = '{type}:{entityId}:{YYYY-MM-DD-local}'.
 */
...
const JOB_NAME    = 'daily-stock-check';
...
const JOB_TIMEOUT_AT = TIMEOUT_MS;
...
export function buildSupplyAlertsProcessor(deps: SupplyAlertsDeps) {
  ...
  return async function process(_job: Job): Promise<void> {
    ...
    if (Date.now() - started > JOB_TIMEOUT_AT) throw new Error(`${JOB_NAME}: timeout`);
    ...
    let totalAlerts = 0, totalLotsUpdated = 0, totalErrors = 0;

    for (const clinic of clinics.rows) {
      // Only run at 06:00 local time (±15-minute window)
      const localHour   = getLocalHour(clinic.timezone);
      const localMinute = getLocalMinute(clinic.timezone);
      if (!(localHour === 6 && localMinute < 15)) continue;

      const todayLocal = getLocalDateString(clinic.timezone);
      const lockKey    = `lock:${JOB_NAME}:${clinic.id}:${todayLocal}`;
      ...
      try {
        ...
        totalErrors += 1;
        log.error({ err, jobName: JOB_NAME, tenantId: clinic.id }, 'supply-alerts: clinic failed');
      } finally {
    ...
    if (totalErrors === 0) metrics.success(durationMs, totalAlerts);
    else metrics.error();

    log.info({
      job_name:        JOB_NAME,
      duration_ms:     durationMs,
      items_processed: totalAlerts,
      items_skipped:   0,
      errors_count:    totalErrors,
      status:          totalErrors > 0 ? 'partial' : 'ok',
      clinics_total:   clinics.rows.length,
      lots_updated:    totalLotsUpdated,
    }, 'supply-alerts: dispatcher sweep complete');
  };
}
...
function getLocalMinute(timezone: string): number {
  try {
    return parseInt(
      new Intl.DateTimeFormat('en-US', { timeZone: timezone, minute: 'numeric' }).format(new Date()),
      10,
    );
  } catch {
